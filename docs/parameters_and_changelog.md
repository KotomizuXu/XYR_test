...
| 题材→审核严格度映射 | strict/flexible 各含题材关键词列表 | — | — | config.yaml `genre_strictness`（#210，从 pipeline.py 硬编码迁移） |
...
| 每批审计章节数 | `5` | — | — | config.yaml `outline_audit.batch_size`（默认复用 Plotter BATCH_SIZE） |

### 用户交互输入（WebSocket session）

| 参数 | 来源 | 位置 |
|------|------|------|
| 故事灵感 `idea` | 👤 用户输入 | web/app.py WebSocket session |
| 小说名称 `name` | 👤 用户输入 | web/app.py WebSocket session |
| 风格描述 `style` | 👤 用户输入（可留空） | web/app.py WebSocket session |
...
| capability_extractor | `0.5` | `style_guide.agent_temperatures.capability_extractor` |
| outline_auditor | `0.3` | `style_guide.agent_temperatures.outline_auditor` |
| outline_global_checker | `0.3` | `style_guide.agent_temperatures.outline_global_checker` |
...
|------|-----------|-----------|------------|---------|
| 角色遗忘阈值 | `10` 章 | `style_guide.suggestions.tracking_thresholds.character` | 自动采纳 AI 推荐值（`_collect_params` 从 `style_guide.suggestions.tracking_thresholds` 读取） | `tracking/config.json` |
| 支线停滞阈值 | `12` 章 | `style_guide.suggestions.tracking_thresholds.plotline` | 自动采纳 AI 推荐值（`_collect_params` 从 `style_guide.suggestions.tracking_thresholds` 读取） | `tracking/config.json` |
| 伏笔回收阈值 | `20` 章 | `style_guide.suggestions.tracking_thresholds.foreshadowing` | 自动采纳 AI 推荐值（`_collect_params` 从 `style_guide.suggestions.tracking_thresholds` 读取） | `tracking/config.json` |
...
映射规则（已从 pipeline.py `_GENRE_STRICTNESS` 硬编码迁移至 config.yaml `genre_strictness` 段，#210）：
...
| 规则 | 定义位置 | 作用 |
|------|---------|------|
| `_BANNED_REPLACEMENTS`（81 个词） | tracker.py | AI 高频词自动替换（如"然而"→"但是"）。已移除 "然而"、"直到"、"仿佛"、"随着"、"首先"、"其次"、"再次"、"不禁"、"宛如" 共 9 个正常文学用词 |
...
| #91 I2 | 工程 | review-rewrite 循环硬上限（≤3 轮重写），到上限后接受当前版本，避免 reviewer 一直 reject 导致无限循环 | `config.yaml novel.review_max_retries: 3` + `core/pipeline.py` 上限日志说明（注：#184 已降为 2，此处为历史记录） |
...
| 编号 | 严重度 | 问题 | 修复位置 |
|------|--------|------|---------|
| #185 | 严重 | `_refine_block` 在 `_audit_one_batch` 返回值被丢弃，用户精修调整不生效 | core/pipeline.py `_audit_one_batch` |
| #186 | 严重 | `batch_size=5` 硬编码在 pipeline.py，与 Plotter BATCH_SIZE 可能不一致 | core/pipeline.py `_audit_one_batch` + config.yaml `outline_audit.batch_size` |
| #187 | 严重 | `_apply_validation_level` 可能用残缺数据覆盖完整 `validation_rules.json` | core/pipeline.py `_apply_validation_level` |
| #188 | 严重 | 禁用词替换过于激进（"然而"/"直到"/"仿佛"/"随着"/"首先"等正常文学词被替换） | core/tracker.py `_BANNED_REPLACEMENTS` |
| #189 | 警告 | 前端 `PromptChoice.vue` 未实现 `allow_custom` 自定义输入 | frontend PromptChoice.vue |
| #190 | 警告 | `agent_map` 中 `style_advisor`/`critic` 键重复定义 | core/pipeline.py `_apply_style_temperatures` |
| #191 | 警告 | `styling`/`collecting_params`/`directing` 三阶段无 try/except 保护 | core/pipeline.py `_run_pipeline` |
| #192 | 警告 | `refining` 阶段在注释中列出但无入口，旧数据 resume 静默终止 | core/pipeline.py `_run_pipeline` + web/routers/novels.py |
| #193 | 警告 | `state.chapters[i]` 无越界保护 | core/pipeline.py `_write_chapters` / `_edit_chapters` |
| #194 | 警告 | 3 个审计 Agent 缺少 config.yaml 独立温度配置 | config.yaml `agents` |
| #195 | 警告 | `update_tracking` 不维护 `characterGroups`（active→inactive） | core/tracker.py `update_tracking` |
| #196 | 警告 | 支线检测只检查 `currentNode` 是否为空，不检查时间距离 | core/tracker.py `check_forgotten` |
| #197 | 警告 | `last_seen=0` 的角色永远不会被检测为遗忘 | core/tracker.py `check_forgotten` |
| #198 | 警告 | `_init_validation_rules` 缺少 `active_validation_level` 初始化 | core/tracker.py `_init_validation_rules` |
| #199 | 警告 | `set_threshold` 不补全 `retired`/`strictness` 字段 | core/tracker.py `set_threshold` |
| #200 | 警告 | `param_suggestions` 的 `words_reason`/`reward_desc` 前端未展示 | frontend ParamTable.vue |
| #201 | 警告 | `PHASE_ORDER` 前端缺 `editing` 标签 | frontend NovelDetailView.vue |
| #202 | 清理 | 删除 ~400 行旧增量导演流程死代码 | core/pipeline.py 行 970-1381 |
| #203 | 清理 | 删除 `from pathlib import Path` 重复导入 + 未使用的 refine_prompts import | core/pipeline.py |
| #204 | 清理 | `_normalize_volume_ranges` 丢弃多余卷时无提示 | core/pipeline.py |
| #205 | 优化 | `get_tracking_context` 硬截断改为按 `##` section 粒度智能截断，避免切断 markdown 格式 | core/tracker.py `get_tracking_context` |
| #206 | 优化 | `analyze_development` 循环内 4 次 `_read_json` 同一文件，改为开头读一次、末尾统一写回 | core/tracker.py `analyze_development` |
| #207 | Bug | 伏笔遗忘检测 `if planted_ch` 在 `planted_ch=0` 时 falsy 跳过检测，改为 `is not None` | core/tracker.py `check_forgotten` |
| #208 | 清理 | `VolumeDef` 延迟 import（tracker.py + pipeline.py 函数体内）移至模块顶部 | core/tracker.py + core/pipeline.py import 区 |
| #209 | 优化 | `query_relevant` 返回值无总长度限制，添加 4000 字符兜底截断 | core/tracker.py `query_relevant` |
| #210 | 配置 | `_GENRE_STRICTNESS` 硬编码字典迁移至 config.yaml `genre_strictness` 段 | config.yaml + core/pipeline.py |
| #211 | 清理 | 删除 `show_novel_list` 死代码（后端 ui.py 定义 + 前端过滤） | core/ui.py + frontend NovelDetailView.vue |
| #212 | 前端 | progress 消息的 `info` 字段前端未渲染，已补充显示 | frontend MessageLog.vue |
| #213 | 前端 | `param_confirmed` 的 `thresholds`（遗忘检测阈值）前端未渲染，已补充显示 | frontend MessageLog.vue |
| #214 | 前端 | `batch_audit` 的 `relationship_summary`（关系网络变化）前端未渲染，已补充显示 | frontend BatchAuditView.vue |
