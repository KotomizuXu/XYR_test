...
PlotAgent      → 章节拆分 + 剧情要点 + 场景结构 + 情绪设计 + 张力管理 + 时空标记
    ⇅
大纲审计 Engine A→D（拆章后自动校验+重写）
  A: CapabilityExtractor   → 角色能力矩阵 + 世界规则矩阵 + 地点约束矩阵
  B+C: OutlineAuditor      → 逐章交叉校验（6项）+ 5维质量评分（50分制）
  D: OutlineGlobalChecker  → 遗忘曲线 + 节奏曲线 + 大纲完整性 + 跨批次一致性
  未通过 → PlotAgent.regenerate_chapters 重写 → 重新审计（最多 2 轮）
    ↓
...
- **多角色协作**：10 个 Agent 各司其职——风格顾问→导演→编剧→（大纲审计三引擎）→作家→审稿→编辑→修订顾问的完整出版流程
- **Director 全量精修**：导演一次性产出世界观 + 角色 + 地点 + 大纲后，用户对完整 JSON 走 "是/调整/重写" 三选一循环，每次调整 LLM 返回完整 JSON 确保跨 block 一致性，打磨满意后再进入剧情拆章
- **风格指南确认**：StyleAdvisor 生成风格指南后，用户走"是/调整/重写"三选一确认循环再进入下一阶段，每次调整自动落盘
- **大纲审计自动重写**：Plotter 拆章后经 Engine A→D 四引擎校验（能力超标/世界规则/伏笔断裂/节奏失衡/大纲覆盖），不通过则 Plotter 重写（温度+0.25、邻居锚定、相似度+质量停滞防护），最多 2 轮，仅在自动重写后仍有问题时让用户决策
...
│   ├── plotter.py           # 编剧：章节拆分 + 剧情规划 + 审计反馈重写
│   ├── capability_extractor.py  # 大纲审计 Engine A：能力/世界规则/地点矩阵提取
│   ├── outline_auditor.py       # 大纲审计 Engine B+C：逐章交叉校验 + 质量评分
│   ├── outline_global_checker.py # 大纲审计 Engine D：遗忘/节奏/完整性/跨批次检查
...
│   ├── director_system.txt
│   ├── plotter_system.txt
│   ├── capability_extract_system.txt   # 大纲审计 Engine A
│   ├── outline_audit_system.txt        # 大纲审计 Engine B+C
│   ├── outline_global_check_system.txt # 大纲审计 Engine D
│   ├── outline_rewrite_system.txt      # 大纲审计重写专用
│   ├── writer_system.txt
│   ├── reviewer_system.txt
│   ├── editor_system.txt
│   ├── critic_system.txt
...
- **边拆边审重构**：原「全部拆完再统一审」改为 PlotAgent 每拆 5 章 → 立即审这 5 章 → 重写修正 → 修正版作为上下文喂给下一批拆分，保证前序章节的审计修正影响后续拆分
  - `core/pipeline.py`：抽 `_audit_one_batch`（单批审计+重写循环），`_run_outline_audit` 瘦身改名 `_finalize_outline_audit`（只留 Engine D3+D4 全局检查）
  - `agents/plotter.py`：`on_batch_complete` 签名扩展为 `(all_plans, batch_start_idx, batch)`，pipeline 用闭包在每批回调内触发审计
  - 关键机制：审计重写后原地 splice 回 plotter 的 all_plans 引用，下一批拆分自动带修正版
  - 断点恢复对齐：崩溃落在「拆完」与「审完」之间时，截断 chapter_plans 到已审章数，重拆+重审尾部批次
- **审计修复**：`OutlineGlobalChecker` 补 `run` 抽象方法（原缺失导致 plotting 阶段 TypeError 崩溃）
- **持久化展示**：新增 `BatchAuditView.vue`/`GlobalAuditView.vue` 子组件（MessageLog 与 NovelDetailView 共用）；plotting Tab 章节列表下方新增批次审计/全书完整性折叠区，刷新页面仍可见
- **大纲入口**：plotting Tab 顶部新增「查看大纲」折叠入口，复用 RefineBlockViewer

### 2026-05-28 详情页刷新按钮 header-extra 不渲染修复

- 修复 NovelDetailView 各 Tab"刷新数据"按钮完全不显示：Naive UI `n-card` 仅在有 `title`/`#header`/`closable` 之一时才渲染 header 区块，而 `#header-extra` 嵌在 header 内；styling/collecting_params/directing/writing 四张无 title 的卡片导致按钮被静默丢弃（同时也修复了一直隐藏的"回滚到此阶段"按钮）
- 四张卡片补 `:title="phaseLabel.xxx"`；plotting Tab 刷新按钮移出 `phaseState==='past'` 限制改为始终显示（回滚按钮仍仅 past 显示）
- 提醒：改 `.vue` 后须 `npm run build` 重建 `frontend/dist/`，`web/app.py` 服务的是打包产物而非 dev server

### 2026-05-28 Plotting 阶段大纲审计系统（四引擎架构）

- 新增大纲审计四引擎架构：Engine A（能力矩阵提取）→ Engine B+C（逐章交叉校验+质量评分）→ Engine D（全局级检查）
- 新增 3 个 Agent：`CapabilityExtractor`、`OutlineAuditor`、`OutlineGlobalChecker`
- 新增 3 个 system prompt：`capability_extract_system.txt`、`outline_audit_system.txt`、`outline_global_check_system.txt`
- Engine B 六项交叉校验：角色能力超标/世界规则违反/地点一致性/关系进度/三幕结构对齐/伏笔完整性
- Engine C 五项质量评分：plot_progression/pacing/opening_hook/ending_hook/character_depth（50分制）
- Engine D 四项全局检查：遗忘曲线/节奏曲线/大纲完整性扫描/跨批次一致性
- 前端 MessageLog.vue 新增 3 种结构化审计渲染器（chapter_audit/batch_audit/global_audit）
- 前端 Plotting Tab 每章卡片新增"审计结果"折叠面板，展示能力矩阵摘要和问题列表
- REST API `GET /novels/{name}` 新增 `capability_matrix`/`chapter_audits`/`batch_audits`/`global_audit` 字段
- `NovelState` 新增 4 个审计数据字段，支持断点恢复时跳过已审计批次
- `config.yaml` 新增 `outline_audit` 配置段（重试次数/默认能力分/质量阈值/节奏规则）
- **新增审计自动重写机制**：`_run_outline_audit` 加入审计→重写→再审循环，完全对齐 Writer review-reject loop
  - `Plotter.regenerate_chapters()`：温度提升 +0.25、7 条重写规则、上下文截断 10000 字符、JSON 字段完整性校验
  - 每轮重写保存版本文件 `chapter_plans_XX_rN.json`，审计结果记录 `revision_count`
  - 防御机制：相似度检测（>0.92 触发 stale 升温）、质量停滞检测（连续2轮不提升则 break）
  - 一致性保证：独立重写+邻居锚定+批次级联审计，级联问题升级为组重写
  - 用户反馈节点：每5章仅在发现问题时暂停（方案C），全部通过自动继续
- 新增 `outline_rewrite_system.txt`：大纲重写专用 system prompt
- `config.yaml` `outline_audit` 段新增 5 个重写控制参数：`audit_max_retries=2`、`stale_similarity_threshold=0.92`、`auto_rewrite=true`、`plotter_rewrite_ctx_cap=10000`、`rewrite_temperature_boost=0.25`

### 2026-05-28 Styling 阶段增加用户确认循环

- Phase 0 (Styling) 接入 `_refine_block` 三选一确认循环（yes/adjust/rewrite），用户可查看、调整或重写风格指南后再进入下一阶段
- 新增 `REFINE_STYLE_PROMPT`（`core/refine_prompts.py`），风格指南精修专用 system prompt
- 新增 `_build_style_context`（`pipeline.py`），为精修构建故事火花 + 用户风格描述的上下文摘要
- 支持断点恢复：中断后再次进入 styling 阶段时跳过重新生成，直接进入精修循环
- `on_update` 回调每次调整后自动落盘 `style_guide`，防止进程崩溃丢失调整进度
